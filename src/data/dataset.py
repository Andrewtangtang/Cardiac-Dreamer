...
class CrossPatientTransitionsDataset(Dataset):
	...
	def __init__(
		...
		# Enforce explicit patient lists per split (no ratio-based auto split)
		if train_patients is None or val_patients is None:
			raise ValueError(
				"Explicit patient lists are required. Provide train_patients and val_patients "
				"via script arguments or config (data.patients.train / data.patients.val)."
			)
		# test_patients can be None (optional test split)
		...
		if split == "train":
			...
			self.patient_ids = test_patients or []
		else:
